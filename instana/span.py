...
class BaseSpan(object):
    ...
    def _validate_attributes(self, attributes):
        ...
        for key in attributes.keys():
            validated_key, validated_value = self._validate_attribute(key, attributes[key])
    ...
    def _validate_attribute(self, key, value):
        ...
        try:
            ...
            if isinstance(key, (six.text_type, six.string_types)):
                ...
                logger.debug("(non-fatal) attribute names must be strings. attribute discarded for %s", type(key))
        except Exception:
    ...
    def _convert_attribute_value(self, value):
        ...
        try:
            ...
            final_value = "(non-fatal) span.set_attribute: values must be one of these types: bool, float, int, list, " \
...
class SDKSpan(BaseSpan):
    ...
    def __init__(self, span, source, service_name, **kwargs):
        ...
        self.data["sdk"]["custom"]["attributes"] = self._validate_attributes(span.attributes)
...
class RegisteredSpan(BaseSpan):
    ...
    def __init__(self, span, source, service_name, **kwargs):
        ...
        if len(span.attributes) > 0:
            self.data["custom"]["attributes"] = self._validate_attributes(span.attributes)
    ...
    def _populate_entry_span_data(self, span):
        if span.name in self.HTTP_SPANS:
            ...
            self.data["lambda"]["functionName"] = span.attributes.pop('lambda.name', "Unknown")
            self.data["lambda"]["functionVersion"] = span.attributes.pop('lambda.version', "Unknown")
            ...
            if trigger_type in ["aws:api.gateway", "aws:application.load.balancer"]:
                ...
                self.data["lambda"]["cw"]["events"]["id"] = span.attributes.pop('data.lambda.cw.events.id', None)
                self.data["lambda"]["cw"]["events"]["more"] = span.attributes.pop('lambda.cw.events.more', False)
                self.data["lambda"]["cw"]["events"]["resources"] = span.attributes.pop('lambda.cw.events.resources', None)

            elif trigger_type == 'aws:cloudwatch.logs':
                self.data["lambda"]["cw"]["logs"]["group"] = span.attributes.pop('lambda.cw.logs.group', None)
                self.data["lambda"]["cw"]["logs"]["stream"] = span.attributes.pop('lambda.cw.logs.stream', None)
                self.data["lambda"]["cw"]["logs"]["more"] = span.attributes.pop('lambda.cw.logs.more', None)
                self.data["lambda"]["cw"]["logs"]["events"] = span.attributes.pop('lambda.cw.logs.events', None)

            elif trigger_type == 'aws:s3':
                self.data["lambda"]["s3"]["events"] = span.attributes.pop('lambda.s3.events', None)
            elif trigger_type == 'aws:sqs':
                self.data["lambda"]["sqs"]["messages"] = span.attributes.pop('lambda.sqs.messages', None)
            ...
            self.data["celery"]["retry-reason"] = span.attributes.pop('retry-reason', None)
    ...
    def _populate_exit_span_data(self, span):
        if span.name in self.HTTP_SPANS:
            ...
            self.data["cassandra"]["cluster"] = span.attributes.pop('cassandra.cluster', None)
            self.data["cassandra"]["query"] = span.attributes.pop('cassandra.query', None)
            self.data["cassandra"]["keyspace"] = span.attributes.pop('cassandra.keyspace', None)
            self.data["cassandra"]["fetchSize"] = span.attributes.pop('cassandra.fetchSize', None)
            self.data["cassandra"]["achievedConsistency"] = span.attributes.pop('cassandra.achievedConsistency', None)
            self.data["cassandra"]["triedHosts"] = span.attributes.pop('cassandra.triedHosts', None)
            self.data["cassandra"]["fullyFetched"] = span.attributes.pop('cassandra.fullyFetched', None)
            self.data["cassandra"]["error"] = span.attributes.pop('cassandra.error', None)
            ...
            self.data["couchbase"]["hostname"] = span.attributes.pop('couchbase.hostname', None)
            self.data["couchbase"]["bucket"] = span.attributes.pop('couchbase.bucket', None)
            self.data["couchbase"]["type"] = span.attributes.pop('couchbase.type', None)
            self.data["couchbase"]["error"] = span.attributes.pop('couchbase.error', None)
            self.data["couchbase"]["error_type"] = span.attributes.pop('couchbase.error_type', None)
            ...
            service = "%s:%s" % (span.attributes.pop('host', None), span.attributes.pop('port', None))
            namespace = "%s.%s" % (span.attributes.pop('db', "?"), span.attributes.pop('collection', "?"))
            ...
            self.data["gcs"]["sourceBucket"] = span.attributes.pop('gcs.sourceBucket', None)
            self.data["gcs"]["sourceObject"] = span.attributes.pop('gcs.sourceObject', None)
            self.data["gcs"]["sourceObjects"] = span.attributes.pop('gcs.sourceObjects', None)
            self.data["gcs"]["destinationBucket"] = span.attributes.pop('gcs.destinationBucket', None)
            self.data["gcs"]["destinationObject"] = span.attributes.pop('gcs.destinationObject', None)
            self.data["gcs"]["numberOfOperations"] = span.attributes.pop('gcs.numberOfOperations', None)
            ...
            for event in span.events:
                if "message" in event.attributes:
                    self.data["event"]["message"] = event.attributes.pop("message", None)
                if "parameters" in event.attributes:
                    self.data["event"]["parameters"] = event.attributes.pop("parameters", None)
        else:
