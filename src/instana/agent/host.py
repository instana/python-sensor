...
class HostAgent(BaseAgent):
    ...
    def report_metrics(self, payload: Dict[str, Any]) -> Union[Response, None]:
    ...
    def report_profiles(self, payload: Dict[str, Any]) -> Union[Response, None]:
        profile_count = len(payload["profiles"])
        if profile_count > 0:
            logger.debug(f"Reporting {profile_count} profiles")
            response = self.client.post(
                self.__profiles_url(),
    ...
    def report_spans(self, payload: Dict[str, Any]) -> Union[Response, None]:
        filtered_spans = self.filter_spans(payload["spans"])
        if filtered_spans:
