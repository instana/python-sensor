...
class RegisteredSpan(BaseSpan):
    ...
    def _populate_entry_span_data(self, span: "InstanaSpan") -> None:
        if span.name in HTTP_SPANS:
            ...
            self._collect_aioamqp_attributes(span)

        elif span.name == "aws.lambda.entry":
            self._collect_lambda_attributes(span)

        elif span.name == "celery-worker":
            self._collect_celery_attributes(span)

        elif span.name == "gcps-consumer":
            self._collect_gcps_consumer_attributes(span)

        elif span.name == "rabbitmq":
            self._collect_rabbitmq_attributes(span)

        elif span.name == "rpc-server":
            self._collect_rpc_attributes(span)
    ...
    def _populate_exit_span_data(self, span: "InstanaSpan") -> None:
        if span.name in HTTP_SPANS:
            ...
            self._collect_boto3_attributes(span)

        elif span.name == "cassandra":
            self._collect_cassandra_attributes(span)

        elif span.name == "celery-client":
            self._collect_celery_attributes(span)

        elif span.name == "couchbase":
            self._collect_couchbase_attributes(span)
            ...
            self._collect_redis_attributes(span)

        elif span.name == "rpc-client":
            self._collect_rpc_attributes(span)

        elif span.name == "s3":
            self._collect_s3_attributes(span)

        elif span.name == "sqlalchemy":
            self._collect_sqlalchemy_attributes(span)
            ...
            self._collect_mongo_attributes(span)

        elif span.name == "gcs":
            self._collect_gcs_attributes(span)

        elif span.name == "gcps-producer":
            self._collect_gcps_producer_attributes(span)

        elif span.name == "log":
            self._collect_log_attributes(span)
    ...
    def _collect_aioamqp_attributes(self, span: "InstanaSpan") -> None:
        self.data["amqp"]["command"] = span.attributes.pop("amqp.command", None)
        self.data["amqp"]["routingkey"] = span.attributes.pop("amqp.routing_key", None)
        self.data["amqp"]["connection"] = span.attributes.pop("amqp.connection", None)
        self.data["amqp"]["error"] = span.attributes.pop("amqp.error", None)

    def _collect_boto3_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_cassandra_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_celery_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_couchbase_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_dynamodb_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_redis_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_rpc_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_s3_attributes(self, span: "InstanaSpan") -> None:
        self.data["s3"]["op"] = span.attributes.pop("s3.op", None)
        self.data["s3"]["bucket"] = span.attributes.pop("s3.bucket", None)

    def _collect_sqlalchemy_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_mysql_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_mongo_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_gcs_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_gcps_consumer_attributes(self, span: "InstanaSpan") -> None:
    ...
    def _collect_gcps_producer_attributes(self, span: "InstanaSpan") -> None:
        self.data["gcps"]["op"] = span.attributes.pop("gcps.op", None)
        self.data["gcps"]["projid"] = span.attributes.pop("gcps.projid", None)
        self.data["gcps"]["top"] = span.attributes.pop("gcps.top", None)

    def _collect_lambda_attributes(self, span: "InstanaSpan") -> None:
