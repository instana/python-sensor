...
class TestBaseCollector:
    ...
    def _resource(self, caplog: LogCaptureFixture) -> Generator[None, None, None]:
        self.collector = BaseCollector(TestAgent())
        yield
    ...
    def test_default(self) -> None:
        assert isinstance(self.collector.agent, TestAgent)
        assert self.collector.THREAD_NAME == "Instana Collector"
        assert isinstance(self.collector.span_queue, multiprocessing.queues.Queue)
    ...
    def test_default_env_is_prod(self):
        with patch("instana.collector.base.env_is_test", False):
            self.collector = BaseCollector(TestAgent())
    ...
    def test_start_collector_when_agent_is_ready(
        ...
        if not self.collector.started:
            self.collector.start()
            assert "BaseCollector.start: launching collection thread" in caplog.messages
            assert self.collector.started
            assert self.collector.reporting_thread.daemon
            assert self.collector.reporting_thread.name == self.collector.THREAD_NAME

    def test_start_agent_can_not_send(self, caplog: LogCaptureFixture) -> None:
        with patch(
            "instana.collector.base.BaseCollector.is_reporting_thread_running",
            return_value=False,
    ...
    def test_shutdown(self, caplog: LogCaptureFixture) -> None:
        caplog.set_level(logging.DEBUG, logger="instana")
    ...
    def test_prepare_and_report_data(self) -> None:
        assert self.collector.prepare_and_report_data()
        with patch("instana.collector.base.env_is_test", False):
            assert self.collector.prepare_and_report_data()
    ...
    def test_collect_snapshot(self, caplog: LogCaptureFixture) -> None:
        caplog.set_level(logging.DEBUG, logger="instana")
