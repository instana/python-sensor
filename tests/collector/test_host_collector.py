...
class TestHostCollector:
    ...
    def _resource(self) -> Generator[None, None, None]:
        self.collector = HostCollector(HostAgent())
    ...
    def test_start(self) -> None:
        with patch(
            ...
            assert isinstance(self.collector.helpers[0].collector, HostCollector)
            assert len(self.collector.helpers) == 1
            assert isinstance(self.collector.reporting_thread, threading.Thread)
            self.collector.ready_to_start = False
            assert not self.collector.start()

    def test_prepare_and_report_data(self, caplog: LogCaptureFixture) -> None:
        caplog.set_level(logging.DEBUG, logger="instana")
        self.collector.agent.machine.fsm.current = "wait4init"
        with patch("instana.agent.host.HostAgent.is_agent_ready", return_value=True):
            self.collector.prepare_and_report_data()
        ...
        with patch("instana.agent.host.HostAgent.is_agent_ready", return_value=False):
            assert not self.collector.prepare_and_report_data()
        self.collector.agent.machine.fsm.current = "good2go"
        caplog.clear()
    ...
    def test_should_send_snapshot_data(self) -> None:
        self.collector.snapshot_data_interval = 999999999999
        assert not self.collector.should_send_snapshot_data()

    def test_prepare_payload_basics(
        self, tracer_provider: InstanaTracerProvider
    ) -> None:
        self.create_agent_and_setup_tracer(tracer_provider)
        ...
        assert "plugins", payload["metrics"]
    ...
    def test_prepare_payload_basics_disable_runtime_metrics(
        self, tracer_provider: InstanaTracerProvider
    ) -> None:
    ...
    def test_prepare_payload_with_snapshot_with_python_packages(
        ...
        payload = self.agent.collector.prepare_payload()
        assert payload
        assert "snapshot" in payload["metrics"]["plugins"][0]["data"]
    ...
    def test_prepare_payload_with_autowrapt(
        self, tracer_provider: InstanaTracerProvider
    ) -> None:
    ...
    def test_prepare_payload_with_autotrace(
        self, tracer_provider: InstanaTracerProvider
    ) -> None:
