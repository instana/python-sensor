...
def test_apply_async(celery_app, celery_worker):
    ...
    assert client_span.data["celery"]["error"] is None
    assert client_span.ec is None
    ...
    assert worker_span.data["celery"]["error"] is None
    assert worker_span.data["celery"]["retry-reason"] is None
    assert worker_span.ec is None
...
def test_send_task(celery_app, celery_worker):
    with tracer.start_active_span("test"):
        celery_app.send_task("tests.frameworks.test_celery.add", (1, 2))
