...
class TestGEventAutoTrace(unittest.TestCase):
    ...
    def test_default_patch_all(self):
        ...
        for module_name in self.default_patched_modules:
            self.assertTrue(
                monkey.is_module_patched(module_name), f"{module_name} is not patched"
            )
    ...
    def test_instana_monkey_options_only_time(self):
        ...
        not_patched_modules = (
            m for m in self.default_patched_modules if m not in ("time", "threading")
        )

        for module_name in not_patched_modules:
            self.assertFalse(
                monkey.is_module_patched(module_name),
    ...
    def test_instana_monkey_options_only_socket(self):
        ...
        self.assertTrue(
            monkey.is_module_patched("socket"), "socket module is not patched"
        )
        not_patched_modules = (
            m for m in self.default_patched_modules if m not in ("socket", "threading")
        )
