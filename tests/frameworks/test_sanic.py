...
def server():
    ...
    proc = multiprocessing.Process(target=launch_sanic, args=(), daemon=True)
    proc.start()
    time.sleep(2)
    yield
...
def test_basic_get(server):
    ...
    assert (asgi_span.data['sdk']['custom']['tags']['http.host'] == '127.0.0.1:1337')
...
def test_404(server):
    ...
    assert (asgi_span.ec == None)
    assert (asgi_span.data['sdk']['custom']['tags']['http.host'] == '127.0.0.1:1337')
    assert (asgi_span.data['sdk']['custom']['tags']['http.path'] == '/foo/not_an_int')
    assert ('http.path_tpl' not in asgi_span.data['sdk']['custom']['tags'])
    assert (asgi_span.data['sdk']['custom']['tags']['http.method'] == 'GET')
    assert (asgi_span.data['sdk']['custom']['tags']['http.status_code'] == 404)
    assert ('http.error' not in asgi_span.data['sdk']['custom']['tags'])
    assert ('http.params' not in asgi_span.data['sdk']['custom']['tags'])
...
def test_500(server):
    ...
    assert (asgi_span.data['sdk']['custom']['tags']['http.host'] == '127.0.0.1:1337')
    assert (asgi_span.data['sdk']['custom']['tags']['http.path'] == '/test_request_args')
    assert (asgi_span.data['sdk']['custom']['tags']['http.path_tpl'] == '/test_request_args')
    assert (asgi_span.data['sdk']['custom']['tags']['http.method'] == 'GET')
    assert (asgi_span.data['sdk']['custom']['tags']['http.status_code'] == 500)
    assert (asgi_span.data['sdk']['custom']['tags']['http.error'] == 'Something went wrong.')
    assert ('http.params' not in asgi_span.data['sdk']['custom']['tags'])
...
def test_path_templates(server):
    ...
    assert (asgi_span.ec == None)
    assert (asgi_span.data['sdk']['custom']['tags']['http.host'] == '127.0.0.1:1337')
    assert (asgi_span.data['sdk']['custom']['tags']['http.path'] == '/foo/1')
    assert (asgi_span.data['sdk']['custom']['tags']['http.path_tpl'] == '/foo/<foo_id:int>')
    assert (asgi_span.data['sdk']['custom']['tags']['http.method'] == 'GET')
    assert (asgi_span.data['sdk']['custom']['tags']['http.status_code'] == 200)
    assert ('http.error' not in asgi_span.data['sdk']['custom']['tags'])
    assert ('http.params' not in asgi_span.data['sdk']['custom']['tags'])
...
def test_custom_header_capture(server):
    ...
    # The background Sanic server is pre-configured with custom headers to capture
