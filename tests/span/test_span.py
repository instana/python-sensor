...
class TestSpanStackTrace:
    ...
    def test_add_stack_hard_limit(
        ...
        # Call _add_stack with is_errored=False
        self.span._add_stack(is_errored=False)

        # Check if default is set
        assert span_processor.agent.options.stack_trace_level == "all"

        assert self.span.stack
        assert len(self.span.stack) <= 40  # Hard cap at 40

        stack_0 = self.span.stack[0]
    ...
    def test_add_stack_level_all(
        ...
        span_processor.agent.options.stack_trace_length = 5
        
        # Non-errored span should get stack trace
        self.span._add_stack(is_errored=False)

        assert self.span.stack
        assert len(self.span.stack) <= 5
    ...
    def test_add_stack_level_error_errored(
        ...
        span_processor.agent.options.stack_trace_level = "error"
        span_processor.agent.options.stack_trace_length = 10
        
        # Errored span should get FULL stack trace (no limit)
        self.span._add_stack(is_errored=True)

        assert self.span.stack
        # Should have more than the configured limit since it's errored
        assert len(self.span.stack) >= 10
    ...
    def test_add_stack_trace_if_needed_exit_span(
        ...
        """Test _add_stack_trace_if_needed for EXIT spans."""
        span_name = "redis"  # EXIT span
        self.span = InstanaSpan(span_name, span_context, span_processor)
        
        # Call the method that checks if it's an EXIT span
    ...
    def test_add_stack_trace_if_needed_non_exit_span(
        ...
        """Test _add_stack_trace_if_needed for non-EXIT spans."""
        span_name = "wsgi"  # Not an EXIT span
        self.span = InstanaSpan(span_name, span_context, span_processor)
        
        # Call the method - should not add stack for non-EXIT spans
        self.span._add_stack_trace_if_needed()
    ...
    def test_add_stack_trace_if_needed_errored_span(
        ...
        """Test _add_stack_trace_if_needed detects errored spans."""
        ...
        # Call the method - should detect error and use full stack
        self.span._add_stack_trace_if_needed()
