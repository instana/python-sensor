...
class TestStackTraceConfiguration:
    ...
    def test_span_filter_from_env_vars(self) -> None:
        """Test span filter configuration from environment variables"""
        with patch.dict(
            os.environ,
            {
                "INSTANA_TRACING_FILTER_EXCLUDE_0_ATTRIBUTES": "http.target;/health",
                "INSTANA_TRACING_FILTER_EXCLUDE_1_ATTRIBUTES": "type;redis",
            },
        ):
            self.options = StandardOptions()

            assert self.options.span_filters is not None
            assert self.options.span_filters["deactivated"] is False
            assert len(self.options.span_filters["exclude"]) == 2
            assert len(self.options.span_filters["include"]) == 0

    def test_span_filter_env_precedence_over_yaml(self) -> None:
        ...
        with patch.dict(
            ...
            assert len(self.options.span_filters["exclude"]) == 1
            assert (
                self.options.span_filters["exclude"][0]["name"]
                == "env_filter_exclude_0"
            )

    def test_span_filter_env_with_suppression(self) -> None:
        """Test span filter with suppression flag"""
        with patch.dict(
            os.environ,
            {
            ...
            assert len(self.options.span_filters["exclude"]) == 1
            assert self.options.span_filters["exclude"][0]["suppression"] is False

    def test_span_filter_env_include_rules(self) -> None:
        """Test span filter with include rules"""
        with patch.dict(
            os.environ,
            {
            ...
            assert len(self.options.span_filters["include"]) == 1
            assert len(self.options.span_filters["include"][0]["attributes"]) == 2

    def test_span_filter_env_complex_scenario(self) -> None:
        """Test complex span filter scenario with multiple rules"""
        with patch.dict(
            os.environ,
            {
                "INSTANA_TRACING_FILTER_EXCLUDE_0_ATTRIBUTES": "http.target;/health,/ready;startswith",
                "INSTANA_TRACING_FILTER_EXCLUDE_1_ATTRIBUTES": "type;kafka|kafka.service;internal;contains",
                "INSTANA_TRACING_FILTER_EXCLUDE_1_SUPPRESSION": "false",
                "INSTANA_TRACING_FILTER_INCLUDE_0_ATTRIBUTES": "type;redis|redis.command;GET,MGET",
            },
        ):
            self.options = StandardOptions()
            ...
            assert len(self.options.span_filters["include"]) == 1
            assert (
                self.options.span_filters["exclude"][0]["attributes"][0]["match_type"]
                == "startswith"
            )
            assert self.options.span_filters["exclude"][1]["suppression"] is False
