...
class TestTracestate(unittest.TestCase):
    ...
    def test_update_tracestate(self):
        ...
        self.assertEqual(
            expected_tracestate,
            self.ts.update_tracestate(tracestate, in_trace_id, in_span_id),
        )
    ...
    def test_update_tracestate_more_than_32_members_already(self):
        tracestate = (
            "congo=t61rcWkgMzE,robo=1221213jdfjkdsfjsd,alpha=5889fnjkllllllll,"
        ...
        in_span_id = "1234567890abcdef"
        expected_tracestate = (
            "in=1234d0e0e4736234;1234567890abcdef,congo=t61rcWkgMzE,robo=1221213jdfjkdsfjsd,"
        ...
        actual_tracestate = self.ts.update_tracestate(
            tracestate, in_trace_id, in_span_id
        )
        self.assertEqual(len(tracestate.split(",")), 34)  # input had 34 list members
        self.assertEqual(
            len(actual_tracestate.split(",")), 32
        )  # output has 32 list members, 3 removed and 1 added
        self.assertEqual(expected_tracestate, actual_tracestate)
        self.assertNotIn(
            "gamadeltaepsilonpirpsigma", actual_tracestate
        )  # member longer than 128 characters gets removed
